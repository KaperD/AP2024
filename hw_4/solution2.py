...
def integrate(f, a, b, pool_class, *, n_jobs=1, n_iter=1000):
    ...
    with pool_class() as executor:
        ...
        futures = [executor.submit(process_part, start, min(n_iter, start + iter_step), step, f, a) for start in range(0, n_iter, iter_step)]
        acc = 0
...
def measure(pool_class, n_jobs):
    ...
    assert math.isclose(result, integrate_original(f, a, b, n_jobs=n_jobs, n_iter=n_iter))
    return end - start
